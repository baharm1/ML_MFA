"""
The following code integrates scRNA-seq data with 13C-MIDs of tissues and 
quantifies single cell metabolic fluxes.
...
def myLoss(
    ...
    cellnames_types, geneScale=None, moduleScale=None,
    lamb1=0.2, lamb2=0.2, lamb3=0.2, lamb4=0.2, lamb5=0.2):
...
# config file
config_dir = './serine_13C_scMFA_config.json'
...
patient_path = construct_path(config_dict['patient_dir'])
patient_scRNA_data_files = os.listdir(patient_path / 'patient_scRNA')
patient_MID_data_files = os.listdir(patient_path / 'patient_MID')
patient_celltype_files = os.listdir(patient_path / 'patient_celltypes')
...
for p in range(len(patient_scRNA_data_files)):
    gene_exp_file = patient_path / 'patient_scRNA' / patient_scRNA_data_files[p]
    cellname_type_file = patient_path / 'patient_celltypes' / patient_celltype_files[p]
    patient_enrich_file = patient_path / 'patient_MID' / patient_MID_data_files[p]
    ...
    if config_dict['model_name'] == "serine":
        ULK_complex = geneExpr[['ATG13_a', 'C12orf44_a', 'RB1CC1_a', 
                                'ATG13_n', 'C12orf44_n', 'RB1CC1_n', 
        ...
        geneExpr['ULK_complex_a'] = ULK_complex[['ULK1_2_a','ATG13_a', 
                                                 'C12orf44_a', 'RB1CC1_a']].min(axis=1)
        geneExpr['ULK_complex_n'] = ULK_complex[['ULK1_2_n','ATG13_n', 
                                                 'C12orf44_n', 'RB1CC1_n']].min(axis=1)
