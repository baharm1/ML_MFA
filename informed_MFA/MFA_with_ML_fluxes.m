...
%% Input files
files = dir("patient_input_data");
files = struct2table(files);
patient_sites = files.name(~files.isdir);

mean_files = dir("serine_mean_ML_rel_flux");
mean_files = struct2table(mean_files);
mean_files = mean_files.name(~mean_files.isdir);

file1 = 'two_comp_model.xlsx';
...
parpool(10);
%%
for ps = 1:length(patient_sites)

timestamp_gl = ['timestamp_', datestr(datetime,'yyyymmdd_HHMM'), char(patient_sites(ps))];
filepath_stored_gl = ['output_files/', timestamp_gl];
...
file2 = ['patient_input_data\', char(patient_sites(ps))];
...
[MID_metab, MID, SD] = import_experiment_data(file2, min_sd);
disp('Experimental data imported');

%% Read median fluxes from scFBA-MFA
file3 = ['serine_mean_ML_rel_flux\', char(mean_files(ps))];

mean_ML_flux = readtable(file3);

mean_ML_flux = table2cell(mean_ML_flux);

ML_rxns = mean_ML_flux(:, 1);
mean_ML_flux = cell2mat(mean_ML_flux(:, 2));
...
[varFlag, mx, Aeq, beq, lb, ub] = fmincon_inputs(Sfull, nRev, metab_size, ...
    metab_char, nflux, input_metabs, metabs_remove, UBval, ...
    unlabeled_metabs, mean_ML_flux);

% Generate non-linear constraint equations
[num_nlcons] = IMMeqn_sym_gen(Sfull, metab_char, AM_full, metab_size, ...
    nflux, varFlag, mx, input_metabs, metabs_remove);

% Function to calculate objective value
get_objective = @(x)calc_obj(x, MID_metab, MID, SD, metab_char, ...
    metab_size, nflux, varFlag);
...
A = [];
b = [];
% just for patient 6 uncomment the following lines
% unlabeled serine flux is lower than denovo serine flux in cortex
% A = zeros(1, numel(lb));
% A(1, 9) = -1;
% A(1, 10) = 1;
% b = zeros(1, 1);
...
% trivial solutions)
lb(1:numel(rxn_full)) = 0.01;

%% Perform optimization

% Generate random guess
x0 = 1 * rand(numel(lb), niter);
for i = 1:length(ML_rxns)
    ind_rxn = find(ismember(rxn_full, ML_rxns(i)));
    x0(ind_rxn, :) = mean_ML_flux(i);
end
x0(find(ismember(rxn_full, 'SERu == SERg')), :) = 0;
x0(find(ismember(rxn_full, 'SERu == SERa')), :) = 0;

% Define optimization options
opt_options = knitro_options('algorithm', 4, 'honorbnds', 2, ...
    'bar_relaxcons', 0, 'outlev', 2, 'gradopt', 2, ...
    'maxtime_real', 1800, 'opttol', 1e-03, 'opttol_abs', 1e-02, ...
...
for i = 1:niter
    ...
    [xopt(:,i), fval(i), exitE(i)] = knitro_nlp(get_objective, ...
        x0(:,i), A, b, Aeq, beq, lb, ub, get_NL_cons, [], opt_options, []);
...
% Plot the result
[~] = plot_MID(xopt_min, metab_size, nflux, varFlag, metab_char, ...
    unlabeled_metabs, MID_metab, MID, SD);
...
for i = 1:numel(metab_char)
    if (varFlag(i) == 0)
        l = metab_size(i);
        idv_list = [idv_list; cellstr([repmat([char(metab_char(i)), '_'], ...
            2 ^ l, 1), dec2bin(0:(2 ^ l) - 1, l)])];
    end
end
